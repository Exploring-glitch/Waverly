    ...
    useEffect(() => {
        const fetchPosts = async () => {
            ...
            try {
                ...
                const liked = (allPosts || []).filter(post => post.likes && post.likes.includes(user._id));
                setPosts(liked);
            } catch (err) {
        ...
        <div className="page">
            <div style={{ maxWidth: '1000px', margin: '0 auto', width: '100%', padding: '1rem 0' }}>
                <div style={{ display: 'flex', alignItems: 'center', gap: '1rem', marginBottom: '2rem', padding: '0 1rem' }}>
                    ...
                    <div>
                        <h2 style={{ margin: 0, color: '#e7e9ea', fontSize: '1.25rem', fontWeight: '700' }}>Liked Posts</h2>
                        <p style={{ margin: 0, color: '#71767b', fontSize: '0.9rem' }}>{posts.length} Liked</p>
                    </div>
                </div>
                ...
                {posts.length > 0 ? (
                    ...
                    <div style={{ textAlign: 'center', padding: '3rem 1rem', color: '#71767b' }}>
                        <h3 style={{ margin: '0 0 0.5rem 0', color: '#e7e9ea' }}>No liked posts yet</h3>
                        <p style={{ margin: 0 }}>Posts you like will appear here.</p>
                    </div>
                )}
            </div>
        </div>
...
export default LikedPosts_Page;
